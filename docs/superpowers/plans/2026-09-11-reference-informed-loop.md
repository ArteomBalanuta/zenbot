...
- [x] Add failing tests: list snapshot with alice/bob/duplicate identity/nil entry yields exact full count two and users; empty snapshot yields count zero and empty users; gateway end-to-end result contains identical typed data and existing delivery receipts.
...
- [x] Run targeted tests and confirm expected missing-data failure.
- [x] Preserve the source snapshot's typed values in `ListRoomOperation.Apply`; share roster dedup/order helper with formatting, not a text parser. Carry JSON through the command bridge and declare its schema. Clone mutable byte slices at boundaries.
- [x] Run `go test ./internal/listener/snapshot ./internal/command ./internal/agent/tool` and inspect compatibility cases.
- [x] Record tests and changed files for independent review. Do not commit other workers' files.
...
- [x] Add failing provider projection tests using a closed object schema with room/count/users, an optional field, nested array, and a large schema. Assert the actual emitted definition gives result shape while parameters remain unchanged.
...
- [x] Run `go test ./internal/agent/tool/contract` and confirm absent return guidance fails.
- [x] Implement deterministic sorted shallow schema projection, bounded to 512 bytes, explicit omission marker. Preserve valid UTF-8; do not serialize verbose schema descriptions or all validation keywords.
- [x] Run contract and live tests; measure production manifest byte delta. Record exact tests for review.
...
- [x] Add failing forced-budget tests where three tool outputs disappear from optional transcript but their small count facts remain in the required index, including zero and errors; full store is unchanged.
- [x] Add a live-loop test with malformed response plus competing observations: next provider request contains current correction, unchanged original prompt and loop status. Current diagnostic is bounded even for huge IDs/names/arguments; rejected calls never execute.
...
- [x] Run targeted tests; verify baseline loses values/feedback as expected.
- [x] Pass current feedback explicitly via required runtime messages, not prefix inference. Bound rejected-call diagnostic. Reuse observation projection for preview data; shrink previews before receipts; mark omissions and retain retrieval IDs.
- [x] Run `go test ./internal/agent/assemble ./internal/agent/live ./internal/agent/tool/execution`. Preserve atomic native call/result transcript and terminal no-tools behavior.
- [x] Record tests and files for independent review.
...
- [x] Add failing tests for committed, partial, unknown, rejected, skipped and mixed results through a wrapped IncompleteTurnError, and for plain error fallback. Include many receipts and hostile/huge call names to test bounded readable output without payload leakage.
...
- [x] Run targeted tests and verify missing renderer/old generic behavior is the failure.
- [x] Implement receipt-only notice bounded to 2000 runes with explicit omitted-receipt count. Preserve no-evidence generic fallback and context cancellation behavior. Never recommend full replay or claim user-goal success.
- [x] Run `go test ./internal/agent/live ./cmd/zenbot` and independently review production integration.
...
- [x] Document all sources/revisions, pattern problem/mechanism/agency/existing solution/gain/cost/decision and documentation discrepancies. Distinguish observed harness correctness from model inference quality.
- [x] Adjust synthetic provider list fixture to return actual production typed data; do not alter expected tool ordering/conditions or loosen assertions to pass.
- [x] Run `go test ./...`, `go vet ./...`, and race tests for affected agent, command and snapshot packages. Inspect and fix regressions.
- [x] Run opt-in configured-provider trials without real command side effects; retain full JSON traces and settings. Report failures honestly and repeat focused cases to characterize variance, not to erase a failed run.
- [x] Independent final review for spec compliance, regression risk and unjustified complexity. Resolve load-bearing findings and rerun covering tests.
- [x] Complete final before/after report and hand off the verified local changes. No push/deploy/restart.
