...
9. Reprojects context before each follow-up. Current resource status, current repair feedback and the bounded factual result index are required context, even when older call/result pairs are pruned.
...
`Registry.Manifest` returns a deterministic versioned caller-visible inventory, rejects colliding primary intents, hides internal fallbacks, and propagates invalid descriptors. The full local manifest retains the execution contract. Its provider projection includes concise purpose, use/avoid guidance, an example, prerequisites, parameters, delivery semantics, and a schema-derived result-shape hint. Hints show optional fields and shallow nested types, mark omissions, and are bounded to 512 UTF-8-safe bytes; they never change callable parameters. The 64-tool creator-visible base manifest measures 39,793 bytes and is regression-tested at 40 KiB or less; the complete 65-tool invocation including `read_tool_result` measures 40,852 bytes. This is an intentional increase from 32,513/33,534 bytes to expose useful output contracts. Context budgeting uses the complete live manifest.
...
The projector estimates tokens, accounts for the live manifest, reserves output/policy capacity, deduplicates source fingerprints, and retains newer high-priority units. Assistant calls and matching tool results form an atomic optional unit. Trusted policy, the exact newest request, and `ContextInput.RequiredRuntime` are mandatory. Current `TOOL_LOOP_FEEDBACK` is explicitly supplied as required runtime context, not trusted by matching a string prefix. Rejected-call diagnostics are bounded to 1,536 serialized bytes with at most three previews and explicit truncation; they do not create execution receipts. Current `TOOL_LOOP_STATUS` remains last and reports remaining model rounds/calls, tool availability, terminal state, and any unknown-action block.

`CURRENT_TURN_RESULTS_UNTRUSTED_DATA` is a required factual index of original call IDs, tool names, bounded argument previews, statuses, codes, effect states, receipts, and bounded structured data/error previews. It starts with 1,024-byte observation projections and 512-byte argument previews, reducing both to fit before rejecting irreducible required metadata. Generic nested-object projection retains small facts beside oversized fields and marks omitted paths/counts; full results remain separate. This keeps useful facts available even for terminal synthesis when paging tools are disabled. Earlier calls remain discoverable and retained content or arguments can be paged by ID while calls remain. Previews may still become unavailable under extreme pressure; if required receipts cannot fit, assembly fails before provider I/O instead of silently losing them.
...
Remote list snapshots additionally return source-owned `{room,users,count,returnedCount,truncated}` through `OperationResult.Data`, the capture gateway, and the `saturn_list` success payload. Data bytes are copied at callback/ownership boundaries. Count and users share the existing identity deduplication and deterministic roster ordering; an empty roster is a successful zero, not an error. No chat-string parsing or new remote-room route is involved. Only list advertises this typed roster; weather, ping and other concrete command result schemas retain their actual message/receipt fields. Their existing command implementations remain authoritative.
...
The production failure sink uses `live.FailureReply` for an `IncompleteTurnError`: a receipt-only notice explicitly labels the answer incomplete and distinguishes succeeded, failed/not-started, partial, and unknown tool outcomes. It is bounded to 2,000 runes, counts omitted receipts, excludes raw arguments/results/causes, and never suggests replaying the task. No-evidence failures keep the generic message; cancellation keeps the existing no-send behavior. This is an operational failure notice, not a deterministic semantic answer or completion judge.
...
Runtime tests establish structural protocol, receipt, isolation, and budget guarantees. They do not establish universal semantic reliability. Conditional actions, complete multi-part answers, argument selection, and appropriate silence depend on the configured model's reasoning. Live-provider evaluation exposed conditional-task failures with thinking disabled. Thinking is enabled in the tracked examples and local configuration, but reasoning-enabled trials still show intermittent stale-read recovery and compound ordering failures. The retained reference-informed candidate passed 7/8 cases and then 3/3 unchanged recovery repeats; extra freshness-guidance experiments did not establish an improvement and were removed. Use the opt-in side-effect-free evaluation before relying on consequential tasks. See the [latest provider evaluation](docs/evals/2026-09-11-reference-loop-provider.md), [reference teardown/rebuild](docs/2026-09-11-reference-loop-comparison.md), and [original evaluation](docs/evals/2026-09-11-tool-loop-provider.md) for complete evidence and limitations.
