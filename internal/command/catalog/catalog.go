...
var entries = []Entry{
	...
	entry("color", []string{"color"}, model.MODERATOR, agentTool("Set user color", "Force an active user's display color.", "moderation", AgentModerator, positionalArguments(requiredToken("nick", "Active nickname."), requiredToken("color", "Color value, such as 00ff00.")), []string{"user", "color"}, "Use when a moderator specifies both user and color.", "Do not use to change the bot theme.", "Set @jill color to 00ff00", `{"nick":"@jill","color":"00ff00"}`)),
	...
	entry("list", []string{"list"}, model.REGULAR, agentTool("List room users", "List users currently present in a named room, including a remote managed room.", "lookup", AgentPublic, positionalArguments(requiredToken("room", "Room whose current users are needed.")), []string{"room users"}, "Use whenever the caller asks who is currently in a room.", "Do not use database history as a substitute for live room membership.", "Who is currently in lounge?", `{"room":"lounge"}`, runCommandCompatible)),
	...
	entry("ping", []string{"ping", "p"}, model.REGULAR, agentTool("Ping bot", "Measure and report Saturn command response time.", "utility", AgentPublic, emptyArguments(), []string{"bot response time"}, "Use when the caller asks to ping Saturn or measure command latency.", "Do not use to test arbitrary network hosts.", "Ping the bot", `{}`, runCommandCompatible)),
