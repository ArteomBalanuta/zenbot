...
var entries = []Entry{
	...
	entry("prefix", []string{"prefix"}, model.ADMIN, agentTool("Manage command prefixes", "Show the active command prefixes or replace the complete list for subsequent commands. Changes are live only, not persisted.", "administration", AgentAdmin, positionalArguments(optionalString("prefix", "Space-separated command prefix tokens, for example . *. The first is used in help examples. Omit to show the current list.")), []string{"command prefix"}, "Use only when a creator explicitly requests inspecting or changing command prefixes.", "Do not infer or change prefixes during ordinary conversation.", "Accept both . and * as command prefixes", `{"prefix":". *"}`)),
	...
	entry("restart", []string{"restart", "reload", "re"}, model.ADMIN, agentTool("Request host restart", "Request a host restart and acknowledge accepted or coalesced admission. Completion is unconfirmed unless the source result was already finished; this invocation does not promise completed host retirement.", "runtime", AgentAdmin, emptyArguments(), []string{"host"}, "Use only for an explicit creator restart request.", "Do not use as error recovery without creator instruction.", "Request a host restart", `{}`)),
	entry("shutdown", []string{"exit", "quit", "shutdown"}, model.ADMIN, agentTool("Request host shutdown", "Request a host shutdown and acknowledge accepted or coalesced admission. Completion is unconfirmed unless the source result was already finished; this invocation does not stop the whole process, replicas, or agent runtime.", "runtime", AgentAdmin, emptyArguments(), []string{"host"}, "Use only for an explicit creator shutdown request.", "Do not use for stopping the process, a room replica, or the agent runtime.", "Request a host shutdown", `{}`)),
	...
	entry("active", []string{"active", "activity"}, model.MODERATOR, agentTool("User activity", "Display activity statistics for one exact trip.", "moderation", AgentModerator, positionalArguments(requiredToken("identity", "Exact trip value; nicknames are not accepted.")), []string{"user activity"}, "Use when a moderator asks for activity statistics.", "Do not use for current room presence.", "Show activity for trip 595754", `{"identity":"595754"}`)),
	entry("authorize", []string{"authorize", "auth"}, model.MODERATOR, agentTool("Authorize trip", "Send an authorization request for one trip; server application is not confirmed.", "moderation", AgentModerator, positionalArguments(requiredToken("trip", "Trip to authorize.")), []string{"trip authorization"}, "Use when a moderator explicitly authorizes a trip.", "Do not use to assign a role; use access instead.", "Authorize trip 595754", `{"trip":"595754"}`)),
	...
	entry("ban", []string{"ban"}, model.MODERATOR, agentTool("Permanently ban user", "Send a permanent-ban request for one active nickname; server application is not confirmed.", "moderation", AgentPermanentBan, positionalArguments(requiredToken("nick", "Active nickname to ban.")), []string{"user"}, "Use only for an explicit permanent-ban request by a creator.", "Do not use for a temporary removal; use kick instead.", "Ban @raider", `{"nick":"@raider"}`, targetedCommand, runCommandCompatible)),
	entry("captcha", []string{"captcha"}, model.MODERATOR, agentTool("Set captcha", "Send a captcha enable or disable request; server application is not confirmed.", "moderation", AgentModerator, booleanStateArguments("enabled", "Whether captcha must be enabled.", "on", "off"), []string{"room captcha"}, "Use for an explicit captcha state change.", "Do not toggle captcha speculatively.", "Enable captcha", `{"enabled":true}`, runCommandCompatible)),
	entry("color", []string{"color"}, model.MODERATOR, agentTool("Set user color", "Send a display-color request for one active nickname; server application is not confirmed.", "moderation", AgentModerator, positionalArguments(requiredToken("nick", "Active nickname."), requiredToken("color", "Color value, such as 00ff00.")), []string{"user", "color"}, "Use when a moderator specifies both user and color.", "Do not use to change the bot theme.", "Set @jill color to 00ff00", `{"nick":"@jill","color":"00ff00"}`, allowsSilentAction)),
	entry("deauthorize", []string{"deauthorize", "deauth"}, model.MODERATOR, agentTool("Deauthorize trip", "Send a deauthorization request for one trip; server application is not confirmed.", "moderation", AgentModerator, positionalArguments(requiredToken("trip", "Trip to deauthorize.")), []string{"trip authorization"}, "Use when a moderator explicitly revokes a trip's authorization.", "Do not use to remove a registered identity.", "Deauthorize trip 595754", `{"trip":"595754"}`)),
	entry("flair", []string{"flair"}, model.MODERATOR, agentTool("Set user flair", "Send a display-flair request for one active nickname; server application is not confirmed.", "moderation", AgentModerator, positionalArguments(requiredToken("nick", "Active nickname."), requiredToken("flair", "Flair value.")), []string{"user", "flair"}, "Use when a moderator specifies both user and flair.", "Do not use to send styled prose.", "Set @jill flair to trusted", `{"nick":"@jill","flair":"trusted"}`)),
	entry("kick", []string{"kick", "k", "out"}, model.MODERATOR, agentTool("Kick room user", "Send a kick request for one active nickname; server application is not confirmed.", "moderation", AgentModerator, positionalArguments(requiredToken("nick", "Active nickname to kick.")), []string{"user"}, "Use for an explicit room-removal request.", "Do not use for permanent bans or vague criticism.", "Kick @raider", `{"nick":"@raider"}`, targetedCommand, allowsSilentAction)),
	...
	entry("lock", []string{"lock", "lockroom"}, model.MODERATOR, agentTool("Set room lock", "Send a room lock or unlock request; server application is not confirmed.", "moderation", AgentModerator, booleanStateArguments("locked", "Whether the room must be locked.", "on", "off"), []string{"room lock"}, "Use for an explicit room lock-state request.", "Do not use to mute individual users.", "Lock the room", `{"locked":true}`)),
	entry("mute", []string{"mute", "dumb"}, model.MODERATOR, agentTool("Mute user", "Send a mute request for one active nickname; server application is not confirmed.", "moderation", AgentModerator, positionalArguments(requiredToken("nick", "Active nickname to mute.")), []string{"user"}, "Use for an explicit mute request targeting an active user.", "Do not use for kicking or permanent bans.", "Mute @spammer", `{"nick":"@spammer"}`, targetedCommand, runCommandCompatible)),
	entry("nuke", []string{"nuke"}, model.MODERATOR, agentTool("Nuke room", "Snapshot the specified room, send one permanent-ban request per observed user, then send a lock request; successful workflow completion confirms writes and flush, not server application.", "moderation", AgentPermanentBan, positionalArguments(requiredToken("room", "Room to nuke.")), []string{"room"}, "Use only for an explicit permanent-ban request by a creator naming a room.", "Do not use for removing one user.", "Nuke the raid room", `{"room":"raid-room"}`, allowsSilentAction)),
	entry("overflow", []string{"overflow", "shoot", "love", "hug", "kiss"}, model.MODERATOR, agentTool("Overflow user", "Send an overflow request for one active nickname; server application is not confirmed.", "moderation", AgentModerator, positionalArguments(requiredToken("nick", "Active nickname.")), []string{"user"}, "Use when a moderator explicitly requests the overflow action.", "Do not infer this action from ordinary figurative language.", "Overflow @jill", `{"nick":"@jill"}`, allowsSilentAction)),
	entry("register", []string{"reg", "register"}, model.MODERATOR, agentTool("Register identity", "Register a new nickname and trip, or attach one new value to an existing identity. Rejects requests when both values already exist.", "identity", AgentModerator, positionalArguments(requiredToken("nick", "Nickname."), requiredToken("trip", "Trip.")), []string{"nickname", "trip"}, "Use when a moderator explicitly supplies both identity values.", "Do not use for DBZ character registration.", "Register jill with trip abc123", `{"nick":"jill","trip":"abc123"}`)),
	...
	entry("resurrect", []string{"move", "recover", "heal", "resurrect"}, model.MODERATOR, agentTool("Move user between rooms", "Find the nickname in the source-room snapshot and send a kick-to-destination request; successful workflow completion confirms submission and flush, not the move.", "moderation", AgentModerator, positionalArguments(requiredToken("nick", "Nickname to move."), requiredToken("source", "Source room."), requiredToken("destination", "Destination room.")), []string{"user", "source room", "destination room"}, "Use when a moderator explicitly specifies user, source, and destination.", "Do not use for configuring automatic movement.", "Move @jill from purgatory to lounge", `{"nick":"@jill","source":"purgatory","destination":"lounge"}`, allowsSilentAction)),
	...
	entry("shadowban", []string{"shadowban", "sban"}, model.MODERATOR, agentTool("Shadow-ban users", "Save local shadow-ban record(s) for one exact nickname or all active names containing a fragment and, for active matches, separately send kick request(s); local persistence is confirmed, but kick application is not.", "moderation", AgentModerator, shadowBanArguments(), []string{"user"}, "Use for an explicit shadow-ban request.", "Do not use for ordinary kicks or permanent bans.", "Shadow-ban @raider", `{"mode":"exact","target":"@raider"}`, targetedCommand, runCommandCompatible)),
	entry("unbanall", []string{"unbanall", "pardonall"}, model.MODERATOR, agentTool("Remove all permanent bans", "Send an unban-all request; server application is not confirmed.", "moderation", AgentPermanentBan, emptyArguments(), []string{"all permanent bans"}, "Use only for an explicit creator request to clear all bans.", "Do not use to remove one ban or shadow ban.", "Unban everyone", `{}`)),
	entry("unban", []string{"unban"}, model.MODERATOR, agentTool("Remove permanent ban", "Send an unban request for one hash; server application is not confirmed.", "moderation", AgentPermanentBan, positionalArguments(requiredToken("hash", "Banned user hash.")), []string{"user"}, "Use only when a creator explicitly supplies the ban hash to remove.", "Do not use for shadow bans or mutes.", "Unban hash HjkUEWNlIRH35Xk", `{"hash":"HjkUEWNlIRH35Xk"}`, targetedCommand, runCommandCompatible)),
	entry("unmute", []string{"unmute", "undumb"}, model.MODERATOR, agentTool("Unmute user", "Send an unmute request for one hash; server application is not confirmed.", "moderation", AgentModerator, positionalArguments(requiredToken("hash", "Muted user hash.")), []string{"user"}, "Use for an explicit unmute request with a hash.", "Do not use for permanent or shadow bans.", "Unmute hash HjkUEWNlIRH35Xk", `{"hash":"HjkUEWNlIRH35Xk"}`, targetedCommand, runCommandCompatible)),
	...
	entry("lastonline", []string{"lastonline", "seen", "last", "online", "lastseen"}, model.REGULAR, agentTool("Last online", "Display the latest persisted public message and JOINED/LEFT observations across stored rooms for an exact nickname or trip.", "lookup", AgentPublic, positionalArguments(requiredToken("nick", "Exact nickname or trip credential.")), []string{"user presence history"}, "Use when the caller asks for a user's public observation history. If nickname and trip matches refer to different public row sets, the source rejects ambiguity; nicks can resolve a trip's observed names.", "Do not infer current room membership or a continuous session, or select an arbitrary identity after ambiguity.", "When was jill last observed?", `{"nick":"jill"}`, runCommandCompatible)),
	entry("nicks", []string{"nicks", "t2n"}, model.REGULAR, agentTool("Nicknames by trip", "List historical nicknames publicly observed for one exact trip across stored rooms.", "lookup", AgentPublic, positionalArguments(requiredToken("trip", "Exact trip credential.")), []string{"trip", "nicknames"}, "Use when the caller asks which names were publicly observed with a trip.", "Do not infer registered ownership or current room membership.", "List observed names for trip 595754", `{"trip":"595754"}`, primaryIntent("trip_nicknames"))),
