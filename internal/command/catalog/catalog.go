...
var entries = []Entry{
	...
	entry("kick", []string{"kick", "k", "out"}, model.MODERATOR, agentTool("Kick room user", "Kick one active user by nickname.", "moderation", AgentModerator, positionalArguments(requiredToken("nick", "Active nickname to kick.")), []string{"user"}, "Use for an explicit room-removal request.", "Do not use for permanent bans or vague criticism.", "Kick @raider", `{"nick":"@raider"}`, targetedCommand)),
