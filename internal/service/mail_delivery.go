...
func (s *MailService) claimMail(ctx context.Context, id int64, trip string) (string, bool, error) {
	...
	err := repository.WithTx(ctx, s.DB, func(tx *sql.Tx) error {
		// Acquire SQLite's write reservation before reading the claim state,
		// including when this transaction was opened in deferred mode.
		if _, err := tx.ExecContext(ctx, `UPDATE mail SET status=status WHERE id=?1`, id); err != nil {
			return err
		}
		var recipients, status string
		if err := tx.QueryRowContext(ctx, `SELECT receiver,status FROM mail WHERE id=?1`, id).Scan(&recipients, &status); err != nil {
			return err
		}
...
func (s *MailService) setMailAttempt(ctx context.Context, id int64, trip, attempt, state string) error {
	return repository.WithTx(ctx, s.DB, func(tx *sql.Tx) error {
		...
		if err := tx.QueryRowContext(ctx, `SELECT receiver FROM mail WHERE id=?1`, id).Scan(&recipients); err != nil {
			return err
		}
