...
var (
	profileRe      = regexp.MustCompile(`(?is)(?:tell\s+me\s+about|describe|profile|summari[sz]e|analy[sz]e)\s+(?:user\s+named\s+["']?)?@?([\p{L}\p{N}_-]{1,100})`)
	...
	historyRe      = regexp.MustCompile(`(?is)\b(?:messages?|history|activity)\s+(?:of|for|from|by)\s+@?([\p{L}\p{N}_-]{1,100})\b`)
...
var nonNickTerms = map[string]struct{}{"experience": {}, "interface": {}, "research": {}, "behavior": {}, "behaviour": {}, "java": {}, "here": {}, "there": {}, "shakespeare": {}, "rome": {}}

func extractedNick(prompt string) string {
	prompt = strings.ReplaceAll(prompt, "\\_", "_")
	for _, re := range []*regexp.Regexp{profileRe, trailingUserRe, possessiveRe, whoRe, speechRe, historyRe} {
		if m := re.FindStringSubmatch(prompt); len(m) > 1 {
			n := NormalizeNick(m[1])
			if _, excluded := nonNickTerms[strings.ToLower(n)]; !excluded {
				return n
			}
		}
	}
