...
func New(config Config, runner Runner, sink Sink) (*Runtime, error) {
	...
	rt := &Runtime{runner: runner, sink: sink, ctx: ctx, cancel: cancel, jobs: make(chan Invocation, jobCapacity), slots: make(chan struct{}, config.MaxConcurrent), admission: make(chan struct{}, jobCapacity), rooms: make(map[string]*sync.Mutex)}
	...
	for {
		...
		case invocation, ok := <-rt.jobs:
			...
			go func() {
				...
				rt.execute(invocation)
			}()
		}
	}
}

func (rt *Runtime) execute(invocation Invocation) {
	...
	if err != nil || result.ShouldReply() == false || rt.ctx.Err() != nil || rt.sink == nil {
		return
	}
	_ = rt.sink.Deliver(rt.ctx, invocation, result)
}
