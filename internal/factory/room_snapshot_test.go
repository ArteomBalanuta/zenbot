...
func TestNewEngineWithOptionsInstallsRoomSnapshotCoordinator(t *testing.T) {
	...
	req := snapshot.RoomSnapshotRequest{WorkflowID: "wf-1", Author: "admin", SourceChannel: "source", TargetChannel: "target", Operation: snapshot.OperationFunc(func(snapshot.RoomSnapshotContext, snapshot.Snapshot) (snapshot.OperationResult, error) { return snapshot.Success(), nil })}
