...
func NewEngine(etype model.EngineType, c *config.Config, repo repository.Repository) common.Engine {
	...
	if err != nil {
		prefixes, prefixErr := c.CommandPrefixes()
		if prefixErr != nil {
			prefixes = []string{c.CmdPrefix}
		}
		return &core.EngineImpl{Type: etype, Channel: c.Channel, Name: c.Name, Prefix: prefixes[0], Prefixes: prefixes, Password: c.Password, EngineWg: new(sync.WaitGroup), OutMessageQueue: make(chan string, 256), ActiveUsers: map[*model.User]struct{}{}, AfkUsers: map[*model.User]string{}}
	}
	return e
}
...
func NewEngineWithOptions(etype model.EngineType, c *config.Config, repo repository.Repository, opts EngineOptions) (*core.EngineImpl, error) {
	...
	prefixes, err := c.CommandPrefixes()
	if err != nil {
		return nil, fmt.Errorf("command prefixes: %w", err)
	}
	...
	e := core.NewEngineImpl(&core.EngineImpl{Type: etype, Prefix: prefixes[0], Prefixes: prefixes, Channel: c.Channel, Name: c.Name, Password: c.Password,
