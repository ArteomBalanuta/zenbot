...
func TestSqlUtilRow324GroupCNotesParity(t *testing.T) {
	...
	if err := notes.Save(context.Background(), "trip-a", tripANote); err != nil {
		t.Fatalf("save trip-a note: %v", err)
	}
	if err := notes.Save(context.Background(), "trip-b", "trip-b only"); err != nil {
		t.Fatalf("save trip-b note: %v", err)
	}

	list, err := notes.List(context.Background(), "trip-a")
	if err != nil {
	...
	otherTrip, err := notes.List(context.Background(), "trip-c")
	if err != nil {
	...
	if err := notes.Save(context.Background(), quoteTrip, quoteTripNote); err != nil {
		t.Fatalf("save quote-bearing trip: %v", err)
	}
	quoteTripNotes, err := notes.List(context.Background(), quoteTrip)
	if err != nil {
	...
	if err := notes.Clear(context.Background(), "trip-a"); err != nil {
	...
	tripB, err := notes.List(context.Background(), "trip-b")
	if err != nil {
	...
	if err := notes.Clear(context.Background(), `trip-a" OR "1"="1`); err != nil {
