...
func (d *Database) RegisterCharacter(ctx context.Context, name string, now func() int64) (int64, error) {
	...
	err := d.WithTx(ctx, func(tx *sql.Tx) error {
		createdOn := now()
		if err := tx.QueryRowContext(ctx, "SELECT id FROM FINAL TABLE (INSERT INTO dbz_characters(name,level,created_on) VALUES($1,1,CAST($2 AS BIGINT)))", name, strconv.FormatInt(createdOn, 10)).Scan(&id); err != nil {
			return err
		}
