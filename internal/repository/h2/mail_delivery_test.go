...
func TestMailDeliveryFailedFinalizationRetainsUnknown(t *testing.T) {
	...
	seedDeliveryMail(t, d.DB, "trip-a", "PENDING")
	if _, err := d.DB.Exec(`ALTER TABLE mail_delivery ADD CONSTRAINT reject_accept CHECK (state <> 'ACCEPTED')`); err != nil {
		t.Fatal(err)
	}
	sends := 0
...
func TestMailDeliveryClaimFailureSendsNothing(t *testing.T) {
	d := openTestDB(t)
	seedDeliveryMail(t, d.DB, "trip-a", "PENDING")
	if _, err := d.DB.Exec(`ALTER TABLE mail_delivery ADD CONSTRAINT reject_claim CHECK (state <> 'ATTEMPTING')`); err != nil {
		t.Fatal(err)
	}
	sends := 0
