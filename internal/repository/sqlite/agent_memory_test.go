...
func TestAgentMemoryRepositoryRealSQLiteIsolatesBoundsOrdersAndExpires(t *testing.T) {
	db := sqlitefixture.Open(t, "agent-memory")
...
func TestAppendAgentTurnRollsBackConversationWhenEvidenceInsertFails(t *testing.T) {
	db := sqlitefixture.Open(t, "agent-turn-rollback")
	ctx := context.Background()
	if _, err := db.DB.ExecContext(ctx, `CREATE TRIGGER reject_forced_evidence BEFORE INSERT ON agent_tool_memory WHEN NEW.content='"force-failure"' BEGIN SELECT RAISE(ABORT, 'reject evidence'); END`); err != nil {
		t.Fatal(err)
	}
...
func TestAgentMemoryRepositoryRejectsInvalidAndCancelledRequests(t *testing.T) {
	...
	db := sqlitefixture.Open(t, "agent-memory-invalid")
...
func TestAgentMemorySummaryRepositoryRoundTripsLatestUnexpiredSummary(t *testing.T) {
	db := sqlitefixture.Open(t, "agent-memory-summary")
	ctx := context.Background()
