...
func TestDBZRealSQLiteRegisterLevelAndAllocationsSpendExactBalance(t *testing.T) {
	d := openTestDB(t)
...
func TestDBZStatsRealSQLiteReadsJoinedSnapshotByName(t *testing.T) {
	d := openTestDB(t)
	ctx := context.Background()
...
func TestDBZStrengthRealSQLiteRejectsOverspendWithoutMutation(t *testing.T) {
	d := openTestDB(t)
	ctx := context.Background()
	if _, err := d.DB.ExecContext(ctx, "INSERT INTO dbz_characters(name,level,created_on) VALUES(?1,?2,?3)", "goku", 2, 1); err != nil {
		t.Fatal(err)
...
func TestDBZRegistrationRollsBackCharacterWhenStatsInsertFails(t *testing.T) {
	d := openTestDB(t)
	ctx := context.Background()
	if _, err := d.DB.ExecContext(ctx, `CREATE TRIGGER reject_initial_dbz_stats BEFORE INSERT ON dbz_stats WHEN NEW.str <= 1 BEGIN SELECT RAISE(ABORT, 'reject initial stats'); END`); err != nil {
		t.Fatal(err)
	}
...
func TestDBZLevelUpRollsBackLevelWhenRewardFails(t *testing.T) {
	...
	if _, err := d.DB.ExecContext(ctx, `CREATE TRIGGER reject_level_reward BEFORE UPDATE ON dbz_stats WHEN NEW.free_stats = 5 BEGIN SELECT RAISE(ABORT, 'reject reward'); END`); err != nil {
		t.Fatal(err)
	}
	if err := d.LevelUp(ctx, "goku"); err == nil {
