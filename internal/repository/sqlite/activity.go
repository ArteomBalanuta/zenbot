...
// to SQLite UTC timestamps, retaining Sunday as day one.
const activityStatsSQL = `
WITH MessagesPerTrip AS (
    SELECT
        trip,
        CAST(strftime('%w', created_on / 1000.0, 'unixepoch') AS INTEGER) + 1 AS day_number,
        CAST(strftime('%H', created_on / 1000.0, 'unixepoch') AS INTEGER) AS "hour",
