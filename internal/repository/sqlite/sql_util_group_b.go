...
in ('LEFT','JOINED')) order by created_on desc,id desc limit ?;`
	selectLastNMessagesSQLite = `SELECT name,trip,message,created_on FROM messages WHERE (name = ?1 or trip = ?2) and visibility = 'PUBLIC' and (message not
...
// keeping the authorization context and two-field delete private to SQLite.
...
func (d *Database) SaturnLastMessages(ctx context.Context, name *string, trip string, count int) ([]repository.SaturnLastMessage, error) {
	...
	rows, err := d.DB.QueryContext(ctx, selectLastNMessagesSQLite, name, trip, fmt.Sprint(count))
	if err != nil {
		return nil, err
	}
	defer rows.Close()
