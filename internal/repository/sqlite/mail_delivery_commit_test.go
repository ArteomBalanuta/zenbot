...
// The actual SQL and transactions run on SQLite. Only the driver's response after
...
type sqliteTestConnector struct {
	driverImpl driver.Driver
	dsn        string
}

func (c sqliteTestConnector) Connect(context.Context) (driver.Conn, error) {
	return c.driverImpl.Open(c.dsn)
}

func (c sqliteTestConnector) Driver() driver.Driver { return c.driverImpl }
...
func mailDBWithCommitHook(t *testing.T, d *Database, hook func() error) *sql.DB {
	...
	dsn := (&url.URL{Scheme: "file", Path: path}).String() + "?_pragma=foreign_keys(1)&_pragma=busy_timeout(5000)"
	db := sql.OpenDB(mailCommitConnector{sqliteTestConnector{d.DB.Driver(), dsn}, hook})
	t.Cleanup(func() {
